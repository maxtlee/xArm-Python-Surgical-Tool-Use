...
def listen_loop(cmd_queue, listen_stop, on_mic_state, on_heard):
    ...
    if not DEEPGRAM_API_KEY:
        on_mic_state("ERROR: DEEPGRAM_API_KEY not set. Check .env file.")
        return
    try:
    ...
    if dg_connection.start(options) is False:
        on_mic_state("ERROR: Deepgram WebSocket connection failed. Check API key and internet.")
        return
...
def execute_loop(arm, gripper, cmd_queue, exec_stop, on_active, on_done, on_log, robot=None, dispatcher=None):
    """
    ...
    while not exec_stop.is_set():
        ...
        if is_gripper_cmd:
        ...
        elif arm is None and robot is None:
            on_log(f"[DEMO] Would execute: {label}")
            import time; time.sleep(0.4)
            ...
            try:
                if cmd[0] == 'stop':
                    ...
                    # For simulation, use move_to_pose or similar if available
                    # Otherwise just move to default position
                    robot.go_home()
                elif cmd[0] == 'move':
                    _, dx, dy, dz, _ = cmd
...
class VoiceControlApp:
    ...
    def _on_demo(self):
        ...
        try:
            ...
            self._log(f"Demo mode — using {self.sim_mode} simulation.")
        except Exception as e:
